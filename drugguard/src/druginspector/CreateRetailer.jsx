...
export default function CreateRetailer() {
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      if (!formData.storeAddress) newErrors.storeAddress = 'Address is required'
      if (!formData.storeArea) newErrors.storeArea = 'Store area is required'
  ...
  const handleSubmit = (e) => {
    ...
    const newErrors = validateStep()
    if (Object.keys(newErrors).length === 0) {
      alert('Retailer account created successfully!')
      setFormData({
        storeName: '', email: '', phone: '', registrationNumber: '',
        pharmacistName: '', pharmacistLicense: '', pharmacistEmail: '',
        storeAddress: '', storeArea: '', temperature: '', humidity: ''
      })
      setStep(1)
    } else {
      setErrors(newErrors)
  ...
  return (
    <div className="p-8 bg-gradient-to-br from-slate-900 to-slate-800 min-h-screen w-full overflow-x-hidden">
      <div className="w-full">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Create Retailer Account</h1>
          <p className="text-slate-400">Complete all steps to register a new retailer</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-slate-800 rounded-lg p-8 border border-slate-700">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2 rounded-lg bg-slate-700 text-white focus:outline-none focus:ring-2 ${
                      errors.storeName ? 'ring-red-500' : 'focus:ring-blue-500'
                    }`}
          ...
          {step === 3 && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-white mb-6">Shop Infrastructure</h2>
              <div className="grid grid-cols-1 gap-6">
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-4 py-2 rounded-lg bg-slate-700 text-white focus:outline-none focus:ring-2 ${
                      errors.storeAddress ? 'ring-red-500' : 'focus:ring-blue-500'
                    }`}
                    placeholder="Enter address"
                    rows="3"
                  />
                  {errors.storeAddress && <p className="text-red-400 text-sm mt-1">{errors.storeAddress}</p>}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">Shop Area (Sq. Ft.) *</label>
                    <input
                      type="number"
                      name="storeArea"
                      ...
                      onChange={handleInputChange}
                      className={`w-full px-4 py-2 rounded-lg bg-slate-700 text-white focus:outline-none focus:ring-2 ${
                        errors.storeArea ? 'ring-red-500' : 'focus:ring-blue-500'
                      }`}
                      placeholder="Enter area"
                    />
                    {errors.storeArea && <p className="text-red-400 text-sm mt-1">{errors.storeArea}</p>}
                  </div>
                  <div>
          ...
          {step === 4 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {['GST Certificate', 'Business License', 'Pharmacist License', 'Shop Photos'].map((doc) => (
