...
export default function SellToCustomer() {
  ...
  const availableDrugs = [
    { id: 1, name: 'Paracetamol 500mg', stock: 500, sellingPrice: 30 },
    { id: 2, name: 'Aspirin 75mg', stock: 300, sellingPrice: 20 },
    { id: 3, name: 'Ibuprofen 400mg', stock: 1000, sellingPrice: 35 },
    { id: 4, name: 'Amoxicillin 250mg', stock: 200, sellingPrice: 50 }
  ]
  ...
  const calculateTotal = () => {
    return formData.drugs.reduce((total, drug) => {
      ...
      return total + (drugInfo ? drugInfo.sellingPrice * (drug.quantity || 0) : 0)
    }, 0).toFixed(2)
  }
  ...
  const handleSubmit = (e) => {
    ...
    setFormData({ customerName: '', customerPhone: '', drugs: [], totalAmount: 0 })
  }
        ...
        <form onSubmit={handleSubmit} className="bg-slate-800 rounded-lg p-8 border border-slate-700">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      ...
                      {availableDrugs.map((d) => (
                        <option key={d.id} value={d.id}>{d.name} (Stock: {d.stock}) - ₹{d.sellingPrice}</option>
                      ))}
                    </select>
                  </div>
                  <div className="w-24">
          ...
          {formData.drugs.length > 0 && (
            <div className="bg-slate-700 rounded-lg p-6 mb-8">
              <h3 className="text-lg font-bold text-white mb-4">Sale Summary</h3>
              <div className="space-y-2 mb-4">
                {formData.drugs.map((drug, idx) => {
                  const drugInfo = availableDrugs.find(d => d.id === parseInt(drug.drugId))
                  if (!drugInfo) return null
                  const amount = drugInfo.sellingPrice * (drug.quantity || 0)
                  return (
